...
CREATE TABLE IF NOT EXISTS `emails` (
  ...
  `custom_headers` text CHARACTER SET utf8 COLLATE utf8_unicode_ci COMMENT 'A serialized array with custom headers to be added when sending with PHPMailer.  If the value is empty string, it is assumed, that the key contains both the key and the value. For more information see the documentation of PHPMailer::addCustomHeader() function.'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
